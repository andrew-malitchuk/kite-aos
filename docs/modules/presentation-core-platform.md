...
- **Motion Sources (by priority)**: For the `Auto` choice, the active source is picked at runtime from the first available of:
    ...
    - **UVC USB webcam** (fallback): a USB webcam that Camera2 does not expose, driven through the `libausbc` UVC engine.
...
### TV Requirements
- A USB webcam (UVC) attached to the box's USB host port, **or** an MQTT presence source, for motion detection.
- The `androidusbcamera` UVC engine is a plain `implementation` on **every** variant (not `tvImplementation`), so the `External` camera source is available to phones/tablets with a USB-OTG webcam as well as TV.
